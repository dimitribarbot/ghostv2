...
class Preprocessor():
    ...
    def preprocess_batched(self, imgs: torch.Tensor, padding_ratio_override: float=None):
    ...
    def make_square_img_batched(self, imgs: torch.Tensor, padding_val: str):
        assert imgs.ndim == 4
        # squarify the image
        h, w = imgs.shape[2:]
        if h > w:
    ...
    def make_padded_img_batched(self, imgs: torch.Tensor, padding: float, padding_val: str):
        if padding == 0:
            return imgs
        assert imgs.ndim == 4
    ...
    def __call__(self, input: torch.Tensor, padding_ratio_override: float=None):
        if input.ndim == 3:
